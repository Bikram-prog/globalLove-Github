...
@php
    $profilepic= DB::Select("SELECT prfl_photo_path FROM users WHERE id = '".Crypt::decryptString($_COOKIE['UserIds'])."'");
  @endphp
        ...
        <div id="profile">
            <div class="wrap">
              @foreach($profilepic as $propic)
                <img id="profile-img" src="{{asset('images')}}/{{ $propic->prfl_photo_path }}" class="online" alt="" />
              @endforeach
                <p><?php echo Crypt::decryptString($_COOKIE['UserFullName']); ?></p>
            ...
            <ul>
                ...
                <li onClick="loadChat('<?php echo $user['user_id']; ?>', '<?php echo $user['name']; ?>', '<?php echo $user['email']; ?>')" id="idd_<?php echo $user['user_id']; ?>" class="contact "> <!-- active -->
                    <div class="wrap">
                        ...
                        <img src="{{asset('images')}}/{{ $user['propic'] }}" alt="" />
                        <div class="meta">
...
var socket = io.connect('https://worldwidemedia.online');
...
function loadChat(str,name,email) {
    $("#namechat").html(name + " <span style='color: #888; font-weight: 600;'>29, Male</span>")
    $("#emchat").val(email)
    $("#hd_t").val(str)
    $(".lds-heart").show();
    ...
    data:{
      ...
      if(msg[1].length > 0) {
        if(msg[1][0].b_who_id == str || msg[1][1].b_who_id == str) {
          $("#msgtxt").hide();
          $("#send").hide();
          $("#attach").hide();  
          $("#blocktxt").html("<p style='text-align: center;margin-bottom: 10px;font-weight: 700; color: #444;'>You can't reply to this conversation.</p>")
        }
      }
