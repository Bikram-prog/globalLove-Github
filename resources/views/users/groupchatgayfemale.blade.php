...
<div class="main_section mt-4">
   <div class="container">
    <div class="row mb-2">
      <div class="col-md-9 text-center">
       <a href="/group-chat" class="btn btn-warning mt-2" style="color: #fff;font-size:15px;margin-left: 20px;">Main Chat</a>

       <a href="/group-chat/gf" class="btn btn-success mt-2" style="color: #fff;font-size:15px;margin-left: 20px;">Lesbian Chat</a>

       <a href="/group-chat/gm" class="btn btn-warning mt-2" style="color: #fff;font-size:15px;margin-left: 20px;">Gay Chat</a>

       <a href="/group-chat/transexual" class="btn btn-warning mt-2" style="color: #fff;font-size:15px;margin-left: 20px;">Transexual Chat</a>
      </div>
      ...
      <div class="row">
        <div class="col-md-12 mt-2">
          ...
          @endphp
          <div class="alert alert-primary" style="font-size: 16px;">{!! $alert[0]->text_alert !!}</div>
        </div>
      </div>
		 <div class="row">
  ...
  @elseif($userapprove[0]->prfl_photo_path == 'https://www.globallove.online/images/male-user-placeholder.png' || str_contains($userapprove[0]->prfl_photo_path, 'google') || str_contains($userapprove[0]->prfl_photo_path, 'facebook'))
      ...
      <div class="chat_bottom">
        <button type="submit" id="myBtn" class="pull-right btn btn-success btn btn-lg">Send</button>
      </div>
      </div>
     </form>
...
window.addEventListener('beforeunload', function(e) {
  ...
  socket.emit('group_online', {
         ...
         socket.on("group_online_offline_status_server", function(data) {
           if(data.data.status == 'online' && data.data.gname == 'Lesbian'){
            $('.toast_notify').show();
          $('.toast_notify').html(`<div style="cursor: pointer; opacity: 1 !important; background: #F15052;color: #fff; width: 100%;" class="toast" role="alert" aria-live="assertive" aria-atomic="true"><div class="toast-header" style="color:#000;"><i class="far fa-comment"></i> &nbsp; <strong class="mr-auto">${data.data.name}</strong><small class="text-muted"> </small><button style="background: transparent;border: none;" type="button" class="ml-2 mb-1" onclick="hidechatbox()"><i class="fas fa-times"></i></button></div><div class="toast-body">${data.data.status}</div></div>`);
