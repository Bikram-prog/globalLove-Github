...
class UsersController extends Controller
{
    ...
    public function home(){
        ...
        return view('users.home')->with(['data'=> $data,'users'=> $users,'testimonial'=> $testimonial,'newmembers'=> $newmembers]);
       }
...
// ORDER BY `users`.`id` DESC LIMIT $offset,$limit");
    ...
    //             ->get();
       ...
       //          ->get();    
        ...
        if(count($data) > 0) {
        	...
        	return view('users.userprofile')->with(['data'=> $data, 'like'=> $users, 'user' => $id, 'heading' => $heading, 'about' => $about, 'prtnr_typ_desc' => $prtnr_typ_desc, 'report' => $report_profile]);
        } else {
    ...
    public function profilesettingspost(Request $request){
		...
		User::where('id',Crypt::decryptString($_COOKIE['UserIds']))->update(['onln_status' => $request->input('onlnstts'), 'display_status' => $request->input('disstts'), 'offline_message' => $request->input('offline_message'), 'subscribe_newsletter' => $request->input('subscribe_newsletter')]);
...
public function viewProfile(){
    ...
    if(!$this->isUserLoginExist()){
       ...
       $about = DB::table('users_metas_status')
        ...
        return view('users.usersprofile')->with(['data'=> $data,'heading'=> $heading,'about'=> $about,'prtnr_typ_desc'=> $prtnr_typ_desc]);
    }
...
setcookie('LookingFor', Crypt::encryptString($request->input('seeking')), time() + (86400 * 365), "/");
 ...
 public function profileDetailsAction(Request $request){
    ...
    public function viewVerifyProfile(){
        ...
        return view('users.verifyprofile');
    }
