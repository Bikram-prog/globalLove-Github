...
use Event;
use App\Events\SignupBroadcast;
...
class UsersController extends Controller
{
    ...
    public function home(){
        ...
        if (!isset($_COOKIE['UserIds'])) {
          ...
          $newmembers = DB::table('users')
                ...
                $fb=$this->facebookLoginAction();

        return view('users.home')->with(['data'=> $data,'users'=> $users,'testimonial'=> $testimonial,'newmembers'=> $newmembers,'newmembershero'=> $newmembershero,'glogin'=> $glogin,'fb'=> $fb]);
       }
    ...
    public function homeTest(){
        ...
        return view('users.homebackup')->with(['data'=> $data,'users'=> $users,'testimonial'=> $testimonial,'newmembers'=> $newmembers,'newmembershero'=> $newmembershero,'glogin'=> $glogin,'fb'=> $fb]);
       }
    ...
    public function viewSuggestions(){
        ...
        return view('users.suggestion_form');
       }
    ...
    public function suggestionUpdatePost(Request $request){
        ...
        return redirect('/suggestion')->with(['success'=>'Your Comments Submitted successfully.']);
    ...
    public function newsUpdatePost(Request $request){
        if(!$this->isUserLoginExist()){
            $this->logout();
            return redirect('/login')->with(['msg' => 'You have to login to view your account']);
        ...
        $request->validate([
            ...
            'news_desc'=>'required|max:5000',
        ],
        [
            'image'=> 'Image type should be jpeg, png, jpg and size less than 2mb', // custom message
            'news_title'=> 'News Title is required and should be maximum of 100 characters', // custom message
            'news_desc'=> 'News Description is required and should be maximum of 5000 characters', // custom message
        ]);
        ...
        DB::table('tdp_news')->insert([
            'news_author' => Crypt::decryptString($_COOKIE['UserFullName']),
            'news_author_id' => Crypt::decryptString($_COOKIE['UserIds']),
            'news_title' => request('news_title'),
            'news_desc' => request('news_desc'),
            'news_img_path' => $imageName
            ...
            DB::table('tdp_news')->insert([
                ...
                'news_author_id' => Crypt::decryptString($_COOKIE['UserIds']),
                'news_title' => request('news_title'),
                'news_desc' => addslashes(request('news_desc'))

            ]);
        }


        return redirect('/news')->with(['success'=>'Post Submitted successfully.']);
    ...
    public function finish($user_id,$domain=''){

        return view('users.finish')->with(['user_id'=> $user_id, 'domain'=> $domain]);

 }

 public function signupActionGoogle(Request $request){
    ...
    if($data[0]->email_verified_at == ''){
      ...
      Mail::send(['html'=>'users.email_verification_template'], ['text'=>$dataa], function($message) use ($mail, $dataa)  {
        ...
        setcookie('UserCountry', Crypt::encryptString($datac[0]->country), time() + (86400 * 365), "/");
        setcookie('LookingFor', Crypt::encryptString(request('looking_fr')), time() + (86400 * 365), "/");
  ...
  $message = "You have a new User on Globallove.";
    ...
    $dataa = User::where('id', '=', [request('id')])->get();

    

        if($request->input('domain') == 'dp') {
            // return redirect('https://thedailyplanet.app');
            
            return response()->json('to_do_smarter_114');     
        } else {
            // event broadcast
            //vent::dispatch(new SignupBroadcast('sam'));

            //return redirect('/profile');

            return response()->json($dataa[0]->name);            
        }
    ...
    public function signup(){
        ...
        return redirect('/browse');
           ...
           $glogin=$this->googleLoginAction();
           $fb=$this->facebookLoginAction();
       	return view('users.signup')->with(['glogin'=> $glogin,'fb'=> $fb]);
       }
    ...
    public function signupaction(Request $request){
        ...
        // } else {
        //     if($request->input('hd_domain') == 'dp') {
        //         $secret   = '6LeEVcEeAAAAAD2XUsCSFqur5-1S_tY0ovt7Z1sK';
        //     } else{
        //         $secret   = '6Lfo2ooeAAAAAGcs0TnNqXFqwt-HBDpzZxks1e7w';
        //     }
    ...
    // //$mail2= 'remorozadarrel@gmail.com';
    //   $dataa2 = "https://www.globallove.online/u/adminlogin";
    //   $datauser = array('name' => $register_info->name,'email' => $register_info->email,'country'  =>$country);
    //   Mail::send(['html'=>'admin.admin_view_users_email_template'], ['text'=>$dataa2,'user'=>$datauser], function($message) use ($mail2, $dataa2)  {
    //      $message->to($mail2)->subject
    //         ('You have a new user.');
    //      $message->from('no-reply@wwwmedia.world','GlobalLove');
    ...
    //         );
            ...
            // ->with(['msg' => 'Please check your email for verify your account.'])

        return redirect("finish/".$register_info->id."/" . $request->input('hd_domain'));
    }
     else{
...
// init configuration
$clientID = '984595876474-hum3siil3pbbg20ed9a663q29btiv1lg.apps.googleusercontent.com';
$clientSecret = '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';
$redirectUri = 'https://globallove.online/g-login-redirect';
...
//   $picture =  $google_account_info->picture;
  $picture =  "https://www.globallove.online/images/male-user-placeholder.png";

$data_user= DB::Select("SELECT t1.*,t2.*,t3.* FROM users t1 LEFT JOIN users_metas t2 ON (t1.id = t2.user_id) LEFT JOIN users_match t3 ON (t1.id = t3.users_match_id) WHERE t1.email = '".$email."'");

if(count($data_user) == 0){
    $ip = $_SERVER['REMOTE_ADDR']; /* Static IP address */
            $currentUserInfo = Location::get($ip);
    ...
    $register_info = new User();
        ...
        $register_info->password = md5(uniqid());
        $register_info->email_verified_at  = date('Y-m-d H:i:s');
        $register_info->token_id = $this->generateRandomString(6);
        $register_info->prfl_photo_path  = $picture;
        $register_info->save();

        $register = new UsersMeta();
        $register->user_id = $register_info->id;
        $register->country = $country;
        $register->save();


  // now you can use this profile info to create account in your website and make user logged in.
  return redirect('finish/'. $register_info->id)->with(['user_id', $register_info->id]);
}else{
    setcookie('UserEmail', Crypt::encryptString($data_user[0]->email), time() + (86400 * 365), "/");
                setcookie('UserFullName', Crypt::encryptString($data_user[0]->name), time() + (86400 * 365), "/");
                setcookie('UserIds', Crypt::encryptString($data_user[0]->id), time() + (86400 * 365), "/");
                setcookie('UserCountry', Crypt::encryptString($data_user[0]->country), time() + (86400 * 365), "/");
                setcookie('LookingFor', Crypt::encryptString($data_user[0]->match_seeking), time() + (86400 * 365), "/");

                $day = date('Y/m/d H:i:s');

                User::where('id', $data_user[0]->id)->update(['login_date_time' => $day]);
...
$facebook = new \Facebook\Facebook([
    'app_id'      => '998174491079255',
    'app_secret'     => 'e3dfeed8222c93b1cb5d08526aac187a',
...
//  $picture = 'http://graph.facebook.com/'.$facebook_user_info['id'].'/picture?type=large';
$picture = 'https://www.globallove.online/images/male-user-placeholder.png';

 $data_user= DB::Select("SELECT t1.*,t2.*,t3.* FROM users t1 LEFT JOIN users_metas t2 ON (t1.id = t2.user_id) LEFT JOIN users_match t3 ON (t1.id = t3.users_match_id) WHERE t1.email = '".$facebook_user_info['email']."'");
...
if(count($data_user) == 0){
    ...
    $register_info = new User();
        $register_info->name = $facebook_user_info['name'];
        $register_info->email  = $facebook_user_info['email'];
        $register_info->password = md5(uniqid());
        $register_info->email_verified_at  = date('Y-m-d H:i:s');
        $register_info->token_id = $this->generateRandomString(6);
        $register_info->prfl_photo_path  = $picture;
...
else
{
 // Get login url
    ...
    $facebook_login_url = $facebook_helper->getLoginUrl('https://globallove.online/fb-login', $facebook_permissions);
    ...
    return $facebook_login_url;
     ...
     public function login($dp=''){
        ...
        if (isset($_COOKIE['UserIds'])) {
           ...
           $glogin=$this->googleLoginAction();
           $fb=$this->facebookLoginAction();
        return view('users.login')->with(['glogin'=> $glogin,'fb'=>$fb, 'dp' => $dp]);
    }
    ...
    public function loginaction(){
        ...
        User::where('id', $data[0]->id)->update(['login_date_time' => $day]);
          ...
          echo "<script>window.ReactNativeWebView.postMessage('loggedin')</script>";
            ...
            if(request('dp') == 'dp') {
                return redirect('https://thedailyplanet.app?auth=true');
            } else {
                return redirect('https://thedailyplanet.app?auth=true&profile=true');
            }
...
// ORDER BY `users`.`id` DESC LIMIT $offset,$limit");
    ...
    public function onlineuser(Request $request){
       ...
       $totalusers =  DB::select("SELECT *
        ...
        LEFT JOIN `users_metas` ON `users_metas`.`user_id` = `users`.`id`
        LEFT JOIN `users_match` ON `users_match`.`users_match_id` = `users`.`id`
        WHERE `users`.`id` != '".Crypt::decryptString($_COOKIE['UserIds'])."' AND `users`.`prfl_approve_status` = '1' AND `users`.`onln_status` = 'online' AND `users_metas`.`age` != '' AND `users`.`id` NOT IN (
        SELECT b_who_id
        FROM block_users
        WHERE b_whom_id = '".Crypt::decryptString($_COOKIE['UserIds'])."')");

        // AND `users_metas`.`sex` = '".Crypt::decryptString($_COOKIE['LookingFor'])."'
...
//        });
    ...
    public function hidePhotoAction($id){
        if(!$this->isUserLoginExist()){
            $this->logout();
            return redirect('/login')->with(['msg' => 'You have to login to view your account']);

        }
        DB::table('profile_post_photos')->where('pro_pic_id', $id)
        ->update([
          'pro_pic_hide_status' => '1'
        ]);

	   return redirect('/profile');
    }

    public function unhidePhotoAction($id){
        if(!$this->isUserLoginExist()){
            $this->logout();
            return redirect('/login')->with(['msg' => 'You have to login to view your account']);
      ...
      public function ajaxRequestPost(Request $request)
    {
       ...
       if(isset($_POST['user_id'])){
        ...
        $attachment = DB::table('like_email_update')->first();

        $mail = $datat[0]->email;
        $dataa = '';
      Mail::send(['html'=>'users.email_like_template'], ['text'=>$dataf[0]->name,'from'=>$user_info], function($message) use ($mail, $dataa, $attachment)  {
         $message->to($mail)->subject
            ('Like Profile');
        ...
        //if(isset($_POST['online_status'])){
            ...
            //offline settings
            if($data[0]->onln_status == 'offline'){
                $attachment = DB::table('offline_email_update')->first();

                    //$mail = $user_info[0]->email;
                    $mail = $data[0]->email;
                    //$mail = "remorozadarrel@gmail.com";
                    $dataa = '';
                Mail::send(['html'=>'users.email_offline_template'], ['to'=>$data,'from'=>$user_info,'msg'=>$msg], function($message) use ($mail, $dataa,$attachment)  {
                    $message->to($mail)->subject
                        ('Message');
                        if($attachment->email_file != ''){
                            $message->attach("http://68.183.224.218/images/".$attachment->email_file);
 ...
 public function mobprofileDetailsAction(Request $request){
    ...
    public function logout() {
      ...
      setcookie("_gooDal", "", time() - 3600);
        ...
        return redirect('https://thedailyplanet.app?auth=false');
    }
...
LEFT JOIN `users_metas` ON `users_metas`.`user_id` = `users`.`id`
LEFT JOIN `users_match` ON `users_match`.`users_match_id` = `users`.`id`
WHERE `users`.`id` != '".Crypt::decryptString($_COOKIE['UserIds'])."' AND `users_metas`.`sex` = '".Crypt::decryptString($_COOKIE['LookingFor'])."'  AND `users`.`prfl_approve_status` = '1' AND `users_metas`.`age` != '' AND `users`.`id` NOT IN ((
SELECT b_who_id
FROM block_users
WHERE b_whom_id = '".Crypt::decryptString($_COOKIE['UserIds'])."'))
ORDER BY `users`.`email` DESC");
...
$totalusers= $totalusers;
    ...
    if (isset($_REQUEST['user_name'])){
        $data = DB::table('users')
                  ...
                  ->where('users.prfl_approve_status', '=', 1)
                  ->where('users_metas.age', '!=', '')
                  ->Where('users.name', 'like', '%' . $request->input('user_name') . '%')
                  ->whereNotIn('users.id', DB::table('block_users')->select('b_who_id')->where('b_whom_id','=', Crypt::decryptString($_COOKIE['UserIds'])))
                  ->orderBy('users.prfl_photo_path', 'asc')
                  ->inRandomOrder()
                  ->groupBy('users.id')
